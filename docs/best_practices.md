...
### 11. Spring Boot MVC for the REST API Layer (Virtual Threads)
...
- **Spring Boot Actuator** for health checks (industry gold standard):
  ...
  - `/actuator/health/readiness` — checks SQLite connection, AI Filtration API reachability, HikariCP pool health
- **JDBCTemplate + HikariCP** for database access — simple, battle-tested, with connection pooling
- **Maven** for dependency management (consistent with the billing service in your GitLab project)
- **Auto-generated OpenAPI docs** via `springdoc-openapi` at `/swagger-ui.html`
...
*   **Lightweight Implementation**:
    - If the Spring Boot gateway receives a medium-risk rating, it instructs Kamailio to return a redirect `rewrite_uri` to an internal IVR route.
