...
- [x] **Kamailio JSON-Response Parsing Hardening (F13)**: ~~Replace regex `allow:false` matching on the gateway HTTP response (`kamailio.cfg` `route[INTERCEPT]`, `$var(res_body) =~ ".*\"allow\"[[:space:]]*:...false.*"`) with structured jansson `json_get_field()` parsing for reliable interception decisions.~~ **IMPLEMENTED**: `kamailio-jansson` added to `configs/kamailio/Dockerfile`; `route[INTERCEPT]`/`route[INTERCEPT_SMS]` use `jansson_get_field("$var(res_body)", "allow", "$var(allow)")` + structural `$var(allow) == 0` (jansson stores JSON boolean `false` as int 0 — the natural `== "false"` string compare was a confirmed silent-dead regression, caught via live runtime error `do_action(): if expression evaluation failed` and fixed). Verified: zero-balance call → 403 live; e2e gate Cell 5 (Kamailio block line + `mvno_sms_blocked_total` counter); robustness probe — boolean false blocks, true/string-"false"/"0"/null fail open.
